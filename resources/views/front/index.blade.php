...
<head>
  ...
  <title>IndNews</title>
  <link rel="icon" type="image/png" sizes="32x32" href="assets/images/logos/favicon/favicon-32x32.png" />
  <!-- <link rel="icon" href="assets/images/logos/logo.svg" type="image/svg+xml" /> -->
...
<body class="font-[Poppins] pb-[72px]">
  <nav id="Navbar" class="max-w-[1130px] mx-auto flex justify-between items-center mt-[30px]">
    <div class="logo-container flex gap-[30px] items-center">
      <a href="/" class="flex shrink-0">
        <img class="w-12 h-12" src="assets/images/logos/logo-indnews.svg" alt="logo" />
      </a>
      ...
      <div class="featured-news-card relative w-full h-[550px] flex shrink-0 overflow-hidden">
        ...
        <div class="card-detail max-w-[1130px] w-full mx-auto flex items-end justify-between pb-10 relative z-20">
          <div class="flex flex-col gap-[10px]">
            ...
            <p class="text-white">{{ $article->updated_at->format('d M Y') }} • {{ $article->category->name }}</p>
          </div>
      ...
      <a href="{{ route('front.author', $author->slug) }}" class="card-authors">
        <div class="rounded-[20px] border border-[#EEF0F7] p-[26px_20px] flex flex-col items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
          ...
          <div class="flex flex-col gap-1 text-center">
            ...
            <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $author->news->count() }} Artikel</p>
          </div>
        </div>
      </a>
      @empty
      <p>Belum ada data penulis terbaru</p>
      @endforelse
    ...
    <div class="flex justify-between items-center h-fit">
      ...
      <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        ...
        <div class="card-detail w-full flex items-end p-[30px] relative z-20">
          <div class="flex flex-col gap-[10px]">
            <p class="text-white">Featured</p>
            <a href="{{ route('front.details', $kesehatan_featured_articles->slug) }}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ substr($kesehatan_featured_articles->name, 0, 40) }}{{ strlen($kesehatan_featured_articles->name) > 40 ? '...' : '' }}</a>
            <p class="text-white">{{ $kesehatan_featured_articles->updated_at->format('d M Y') }}</p>
          </div>
        </div>
      </div>
      @else
      <p>Belum ada artikel unggulan kesehatan terbaru</p>
      @endif
          ...
          <a href="{{ route('front.details', $article->slug) }}" class="card py-[2px]">
            <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
              ...
              <div class="flex flex-col justify-center-center gap-[6px]">
                <h3 class="font-bold text-lg leading-[27px]">{{ substr($article->name, 0, 40) }}{{ strlen($article->name) > 40 ? '...' : '' }}</h3>
                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $article->updated_at->format('d M Y') }}</p>
              </div>
            </div>
          </a>
          @empty
          <p>Belum ada artikel kesehatan terbaru</p>
          @endforelse
  ...
  <section id="Latest-business" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
    ...
    <div class="flex justify-between items-center h-fit">
      ...
      @if ($opini_featured_articles)
      <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        <img src="{{ Storage::url($opini_featured_articles->thumbnail) }}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
        <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
        <div class="card-detail w-full flex items-end p-[30px] relative z-20">
          <div class="flex flex-col gap-[10px]">
            <p class="text-white">Featured</p>
            <a href="{{ route('front.details', $opini_featured_articles->slug) }}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ substr($opini_featured_articles->name, 0, 40) }}{{ strlen($opini_featured_articles->name) > 40 ? '...' : '' }}</a>
            <p class="text-white">{{ $opini_featured_articles->updated_at->format('d M Y') }}</p>
          </div>
        </div>
      </div>
      @else
      <p>Belum ada artikel unggulan opini terbaru</p>
      @endif

      <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
        <div class="w-[455px] flex flex-col gap-5 shrink-0">

          @forelse ($opini_articles as $article)
          <a href="{{ route('front.details', $article->slug) }}" class="card py-[2px]">
            <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
              <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                <img src="{{ Storage::url($article->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail" />
          ...
          @empty
          <p>Belum ada artikel opini terbaru</p>
          @endforelse
  ...
  <section id="Latest-automotive" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
    ...
    <div class="flex justify-between items-center h-fit">
      ...
      @if ($pendidikan_featured_articles)
      <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
        <img src="{{ Storage::url($kesehatan_featured_articles->thumbnail) }}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
        <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
        <div class="card-detail w-full flex items-end p-[30px] relative z-20">
          <div class="flex flex-col gap-[10px]">
            <p class="text-white">Featured</p>
            <a href="{{ route('front.details', $pendidikan_featured_articles->slug) }}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ substr($pendidikan_featured_articles->name, 0, 40) }}{{ strlen($pendidikan_featured_articles->name) > 40 ? '...' : '' }}</a>
            <p class="text-white">{{ $pendidikan_featured_articles->updated_at->format('d M Y') }}</p>
          </div>
        </div>
      </div>
      @else
      <p>Belum ada artikel unggulan pendidikan terbaru</p>
      @endif

      <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
        <div class="w-[455px] flex flex-col gap-5 shrink-0">

          @forelse ($pendidikan_articles as $article)
          <a href="{{ route('front.details', $article->slug) }}" class="card py-[2px]">
            <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
              <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                <img src="{{ Storage::url($article->thumbnail) }}" class="object-cover w-full h-full" alt="thumbnail" />
          ...
          @empty
          <p>Belum ada artikel pendidikan terbaru</p>
          @endforelse
